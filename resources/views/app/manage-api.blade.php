            ...
            <div class="card">
                <div class="card-body">
                    <form novalidate="novalidate" method="post" action="{{route('app.save-settings')}}"
                          id="form-contact" data-form="{{$base_url}}/app/manage-api/{{$api->uuid}}"
                          data-btn-id="btn-save-contact">

                        <h4>{{__('API')}}</h4>

                        <div class="mb-3">
                            <label for="api_name">{{__('Name')}}</label>
                            <input type="text" class="form-control" id="api_name" name="name"
                                   value="{{$api->name ?? ''}}" required>
                        </div>
